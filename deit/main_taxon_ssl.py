...
class CHMatchModule(nn.Module):
    ...
    def forward(self, x):
        if self.is_hier:
            ...
            return (*logits, feats) # including the featus as well to match the HierVisionTransformer
...
def train_one_epoch(model, dataloader, k_value, optimizer, device, epoch, args):
    ...
    for images, labels, species_labels, family_labels, order_labels in metric_logger.log_every(dataloader, 10, header):
        ...
        species_logits, family_logits, order_logits, feats = output
        ...
        # overall loss
        loss_all = loss_supervised + loss_pl + loss_contrastive 
        # backward pass
        optimizer.zero_grad()
        loss_all.backward()
        optimizer.step()
